...
/*    public PatternElem(){
    ...
    public List<PatternElem> defineFuncArguments(String args) throws StringAnalysisException{
        ...
        if(args.length() > 0) { // clear beginning white space
        ...
        while(comma >= 0){
            ...
            if(args.length() > 0 && args.charAt(pos) == '\'') {
                strL = pos;
            ...
            } else if(args.length() > 0 && args.charAt(pos) == '('){
            ...
            } else if(args.length() > 0 && csets.letters.memberOf(args.charAt(pos))) {
                if(braceL > pos) {
            ...
            } else if (args.length() > 0 && csets.digits.memberOf(args.charAt(pos))) {
                pat = new PatternTypeInteger(args);
            }
            pos = stop;
            funcArgs.add(pat);
        }
